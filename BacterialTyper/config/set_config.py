...
def get_exe(prog, Debug=False, Return_Version=False):
    ...
    if prog in os.environ: 
        ...
        try:
            ...
            print("Software: " + prog +" is not included as a dependencies.csv file")
            exit()
    ...
    #else:
    ## get paths
    exe_path_tmp = my_which(exe, Debug)
    ...
    for p in exe_path_tmp:
        ...
        if (not prog_ver):
            if (Debug):
                print (colored("** Debug: Not version available", 'yellow'))
            continue

        if (prog_ver == "."):
            if (Debug):
                print (colored("** Debug: Version available: .", 'yellow'))
            continue

        if LooseVersion(prog_ver) >= LooseVersion(min_version):
            if (Debug):
                print (colored("** Debug: Version OK", 'yellow'))
            if (Return_Version):
                return (p, prog_ver)
            else:
                return (p)
...
def my_which(cmd, Debug=False):
    ...
    if not path:
        if Debug:
            HCGB_aes.debug_message("PATH='' doesn't match, whereas PATH=':' looks in the current directory")
        return None
