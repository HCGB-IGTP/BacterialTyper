...
def getdbs(source, database_folder, option, debug):
    ...
    for option_item in option_list:
        ...
        ## debug message
        if (debug):
            HCGB_aes.debug_message("Option item: " + option_item)
        ...
        ### AMRfinder
        elif (option_item.startswith("AMRfinder")):
            dbs2use = option_item.split(":")[1].split(",")

        ### kraken_user
        elif (option_item.startswith("user_kraken")):
            dbs2use = option_item.split(":")[1].split(",")
            ##     colname = ["source", "db", "path", "timestamp"]
            db_Dataframe.loc[len(db_Dataframe)] = ['kraken', "user_db", dbs2use[0], '']
        ### kraken
        elif (option_item.startswith("kraken")):
            dbs2use = option_item.split(":")[1].split(",")
    ...
    #############
        ...
        ## debug message
        if (debug):
            HCGB_aes.debug_message("Folders KMA_db:" + str(kma_dbs))
        ...
        for db in dbs2use:
            ...
            ## debug message
            if (debug):
                HCGB_aes.debug_message("this_db:" + this_db)
            ...
            if (db == "genbank"):
                ...
                if os.path.isfile(this_db_file + '.comp.b'):
                    HCGB_aes.green_message("\t- genbank: including information from different reference strains available.") ## include data from NCBI
            ...
            #### user_data
                ...
                if os.path.isfile(this_db_file + '.comp.b'):
                    HCGB_aes.green_message("\t- user_data: including information from user previously generated results") ## include user data
                    stamp = time.time() ## to do add time
            ...
            ## default KMA databases: bacteria & plasmids
                ...
                ## debug message
                if (debug):
                    HCGB_aes.debug_message("this_db_file:" + this_db_file)
                ...
                if os.path.isfile(this_db_file + '.comp.b'):
                    ...
                    HCGB_aes.green_message("\t- KMA: including information from database " + db)
                else:
                    HCGB_aes.error_message("\t**KMA: Database %s was not available." %db)
    ...
    #### AMRfinder ####
    ###############
    elif (source == 'AMRfinder'):
        
        ## debug message
        if (debug):
            HCGB_aes.debug_message("Checking:")
            print(dbs2use)

        ### Check if folder exists
        AMR_folder = HCGB_files.create_subfolder('AMRfinder', database_folder)
        latest_fol = os.path.join(AMR_folder, 'latest')

        if latest_fol == dbs2use[0]: ## only main db provided
            ## update
            (code_check_db, stamp) = amrfinder_caller.download_database(AMR_folder, Debug=debug)
            if (code_check_db == 'OK'):
                db_Dataframe.loc[len(db_Dataframe)] = ['AMRfinder', latest_fol, 'main', stamp]
                HCGB_aes.green_message("\t- AMRfinder: including information from main database: " + latest_fol)
            else:
                HCGB_aes.error_message("\t- AMRfinder Failed: database: " + latest_fol)
        
        else:
            ## A different database has been provided not included with other BacterialTyper databses
            AMR_folder = HCGB_files.create_folder(database_folder)
            (code_check_db, stamp) = amrfinder_caller.download_database(AMR_folder, Debug=debug)
            if (code_check_db == 'OK'):
                 db_Dataframe.loc[len(db_Dataframe)] = ['AMRfinder', latest_fol, 'user', stamp]
                 HCGB_aes.green_message("\t- AMRfinder: including information from main database: " + latest_fol)
    ...
    ###############
        ...
        kraken_main = HCGB_files.create_subfolder('Kraken', database_folder)

        k2_db = os.path.join(kraken_main, dbs2use[0])
        
        if k2_db != dbs2use[0]:
            if os.path.isdir(k2_db):
                ## TODO
                ## Check status of Kraken2 db
                #(code_check_db, stamp) = amrfinder_caller.download_database(AMR_folder, Debug=debug)
                code_check_db = "OK"
                stamp = HCGB_time.create_human_timestamp()
                if (code_check_db == 'OK'):
                    ##     colname = ["source", "db", "path", "timestamp"]
                    db_Dataframe.loc[len(db_Dataframe)] = ['kraken', dbs2use[0], k2_db, stamp]
                    HCGB_aes.green_message("\t- Kraken2: including information from database: " + dbs2use[0])
                else:
                    HCGB_aes.error_message("\t- Kraken2 Failed: database: " + latest_fol)
