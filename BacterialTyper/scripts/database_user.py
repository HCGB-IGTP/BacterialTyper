...
def get_userData_files(options, project_folder):
    ...
    ## debug message
    if (options.debug):
        HCGB_aes.debug_message("pd_samples_reads")
        HCGB_main.print_all_pandaDF(pd_samples_reads)
        HCGB_aes.debug_message("pd_samples_assembly")
        HCGB_main.print_all_pandaDF(pd_samples_assembly)
        HCGB_aes.debug_message("pd_samples_annot")
        HCGB_main.print_all_pandaDF(pd_samples_annot)

    ## merge
    df_merge = pd.concat([pd_samples_reads, pd_samples_annot, pd_samples_assembly], 
                   sort=True).drop_duplicates()
    ## convert empty into NaN
    df_merge = df_merge.mask(df_merge == '')
    df_merge = df_merge.dropna(axis=1, how="all") ## only columns with all values equal NaN are remvoed
    
    ## joining by inner we only get common columns among all

    ## debug message
    if (options.debug):
        HCGB_aes.debug_message("df_merge")
...
def get_userData_info(options, project_folder):
    ...
    if (options.debug):
        ...
        HCGB_aes.debug_message("pd_samples_ident")
        HCGB_main.print_all_pandaDF(pd_samples_ident)
        HCGB_aes.debug_message("pd_samples_mash")
