...
by executing ``source env/BacterialTyper/bin/activate``, you should see a tag in the command-line prompt as shown in 
the following image.
...
bcl2fastq :cite:`bcl2fastq` performs base calling and writes the sequence and quality scores into two FASTQ files per 
sample (one for R1 and one for R2), after separating de-multiplexing mixed samples from the library pool by bar code indices. 
